...
from netlink import NulStrAttr
...
class GeNlMessage(Message):
    ...
    @staticmethod
    async def recv(conn, multiple=False):
        msgs = await conn.recv(multiple=multiple)
        genlmsgs = []
        if not multiple:
            msgs = [msgs]
