...
class Message:
    ...
    async def send(self, conn):
        ...
        await conn.send(hdr + self.payload)
...
class Connection:
    ...
    async def send(self, msg: bytes):
        await self.loop.sock_sendall(self.descriptor, msg)

    async def recv(self, multiple=False):
        messages = []
        done = False

        while not done:
            contents = await self.loop.sock_recv(self.descriptor, 16384)
